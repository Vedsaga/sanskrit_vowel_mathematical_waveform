...
def load_patterns(patterns_folder: str) -> Dict[str, PhonemePattern]:
    ...
    for filename in tqdm(os.listdir(patterns_folder), desc="Loading Patterns"):
        ...
        with open(filepath, 'rb') as f:
            ...
            covars = read_np_array(np.float32, (GMM_COMPONENTS, N_FEATURES_MODEL, N_FEATURES_MODEL))
            kmeans_centers = read_np_array(np.float32, (KMEANS_CLUSTERS, N_FEATURES_MODEL))
            ...
            patterns[phoneme_name] = PhonemePattern(
                name=phoneme_name, energy_template=energy, centroid_template=centroid,
                zcr_template=zcr, gmm_weights=weights, gmm_means=means,
                gmm_covariances=covars, kmeans_centers=kmeans_centers,
                transition_matrix=trans_matrix, mfcc_mean=mfcc_mean, mfcc_std=mfcc_std,
...
def extract_features_for_test(filepath: str):
    try:
        ...
        if len(y) < FEAT['frame_length_samples']: return None
        mfccs = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=N_MFCC_TOTAL, n_fft=FEAT['n_fft'], win_length=FEAT['frame_length_samples'], hop_length=FEAT['hop_length_samples'])
        delta_mfccs = librosa.feature.delta(mfccs)
        delta2_mfccs = librosa.feature.delta(mfccs, order=2)
        combined_features = np.hstack([mfccs.T, delta_mfccs.T, delta2_mfccs.T])
        energy = librosa.feature.rms(y=y, frame_length=FEAT['frame_length_samples'], hop_length=FEAT['hop_length_samples'])[0]
        return {'mfccs': combined_features, 'energy': energy}
    except Exception:
        return None

def recognize_phoneme(wav_path: str, all_patterns: Dict[str, PhonemePattern], config: Dict) -> tuple[str, float]:
    ...
    for phoneme_name in candidates:
        ...
        input_mfccs_normalized = (input_mfccs_model - pattern.mfcc_mean) / (pattern.mfcc_std + 1e-8)
...
if __name__ == "__main__":
    ...
    for phoneme_dir in os.listdir(augmented_folder):
        ...
        if not os.path.isdir(dir_path) or not phoneme_dir in patterns: continue

        all_files_in_dir = sorted(os.listdir(dir_path), key=lambda f: int(f.split('_')[2].split('.')[0]))

        # Select exactly 100 files with indices from 1901 to 2000
        files_to_test = [f for f in all_files_in_dir if f.endswith('.wav') and 501 <= int(f.split('_')[2].split('.')[0]) <= 2000]
    ...
    if FULL_RECOGNITION_MODE:
        ...
        with open(results_file, 'w') as f:
            f.write("file_path,actual_phoneme,predicted_phoneme,confidence_score,is_correct\n")

            correct_predictions = 0
            for file_path, actual_phoneme in tqdm(test_files, desc="Evaluating"):
                predicted_phoneme, score = recognize_phoneme(file_path, patterns, CONFIG)
                is_correct = (actual_phoneme == predicted_phoneme)
                if is_correct:
                    correct_predictions += 1
                f.write(f"{file_path},{actual_phoneme},{predicted_phoneme},{score},{is_correct}\n")

        accuracy = (correct_predictions / len(test_files)) * 100 if test_files else 0
        print("\n--- Full Recognition Complete ---")
        print(f"📊 Results saved to: {results_file}")
        print(f"🎯 Overall Accuracy: {accuracy:.2f}% ({correct_predictions}/{len(test_files)})")
        print("---------------------------------")

    else: # Self-Accuracy Mode
        print("🎯 Running in SELF-ACCURACY mode (matching each file against its own pattern)...")
        results_file = "evaluation_results_self_accuracy.csv"
        with open(results_file, 'w') as f:
            f.write("file_path,target_phoneme,self_match_score\n")

            for file_path, actual_phoneme in tqdm(test_files, desc="Evaluating Self-Accuracy"):
        ...
        print(f"📊 Results saved to: {results_file}")
        print("📈 You can now analyze the scores in this file to see how well each pattern matches its own samples.")
        print("-----------------------------------------")
